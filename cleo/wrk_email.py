...
def _send_message(flst_to, fem_message):
    ...
    try:
        ...
        # server handshake
        l_server.starttls()
        l_server.login("DS_YAH_USR", "DS_YAH_PWD")
        l_server.sendmail("DS_YAH_USR", flst_to, fem_message.as_string())
        l_server.quit()
    ...
    # em caso de erro,...
        ...
        # logger
        M_LOG.error("error on email_service", exc_info=1)
...
def send_email(flst_to, fs_token, fv_upload=df.DV_RESULT_UPLOAD):
    ...
    if not fv_upload:
        ...
        with open(file_send, "rb") as lfh:
            ...
            lma_part["Content-Disposition"] = f"attachment; filename='{os.path.basename(file_send)}'"
